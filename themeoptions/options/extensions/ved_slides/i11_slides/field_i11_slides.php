		...
		public function render() {
			...
			if ( isset( $this->value ) && is_array( $this->value ) && !empty( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<span class="button media_upload_button" id="add_' . esc_attr( $x ) . '">' . esc_html__( 'Upload', 'bigbo' ) . '</span>';
					...
					echo '<span class="button remove-image' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $x ) . '" rel="' . esc_attr( $slide['attachment_id'] ) . '">' . esc_html__( 'Remove', 'bigbo' ) . '</span>';

					echo '</div>' . "\n";

					echo '<ul id="' . esc_attr( $this->field['id'] ) . '-ul" class="redux-slides-list">';

					echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-url_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][url]" value="' . esc_attr( $slide['url'] ) . '" class="full-text upload" placeholder="' . esc_html__( 'URL', 'bigbo' ) . '" /></li>';
					echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-title_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][title]" value="' . esc_attr( $slide['title'] ) . '" placeholder="' . esc_html__( 'Title', 'bigbo' ) . '" class="full-text slide-title" /></li>';
					echo '<li><textarea name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][description]" id="' . esc_attr( $this->field['id'] ) . '-description_' . esc_attr( $x ) . '" placeholder="' . esc_attr__( 'Description', 'bigbo' ) . '" class="large-text" rows="6">' . esc_attr( $slide['description'] ) . '</textarea></li>';
					echo '<li><input type="text" id="' . esc_attr( $this->field['id'] ) . '-link_' . esc_attr( $x ) . '" name="' . esc_attr( $this->field['name'] ) . '[' . esc_attr( $x ) . '][link]" value="' . esc_attr( $slide['link'] ) . '" placeholder="' . esc_attr__( 'Slide Link', 'bigbo' ) . '" class="full-text" /></li>';
					...
					echo ' ' . esc_html__( 'Open Link in New Tab/Window', 'bigbo' ) . '</label></li>';
					...
					echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . esc_html__( 'Delete Slide', 'bigbo' ) . '</a></li>';
			...
			if ( $x == 0 ) {
				...
				echo '<span class="button remove-image' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $x ) . '" rel="' . esc_attr( $this->parent->args['opt_name'] ) . '[' . esc_attr( $this->field['id'] ) . '][attachment_id]">' . esc_html__( 'Remove', 'bigbo' ) . '</span>';
				...
				$placeholder = (isset( $this->field['placeholder']['link'] )) ? esc_attr( $this->field['placeholder']['link'] ) : __( 'Slide Link', 'bigbo' );
			...
			/* translators: %s: slide */
			echo '</div><a href="javascript:void(0);" class="button redux-slides-add2 ved_redux-slides-add button-primary" rel-id="' . esc_attr( $this->field['id'] ) . '-ul" rel-name="' . esc_attr( $this->field['name'] ) . '[title][]">' . sprintf( esc_html__( 'Add %s', 'bigbo' ), esc_attr( $this->field['content_title'] ) ) . '</a><br/>';
		}

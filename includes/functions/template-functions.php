...
function daydream_redirect() {
	if ( get_option( 'ved_do_activation_redirect', false ) ) {
		...
		if ( ! isset( $_GET[ 'activate-multi' ] ) ) {
			wp_redirect( "admin.php?page=daydream-menu" );
		}
	}
}
...
function daydream_after_setup() {
	...
	$dd_width_px		 = daydream_get_option( 'dd_width_px', '1200' );
	$dd_custom_width_px	 = daydream_get_option( 'dd_custom_width_px', '1200' );
	if ( $dd_width_px != "custom" ) {
		$dd_width_px	 = apply_filters( 'daydream_header_image_width', $dd_width_px );
		//define( 'HEADER_IMAGE_WIDTH', apply_filters( 'daydream_header_image_width', $dd_width_px ) );
		//define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'daydream_header_image_height', 170 ) );
		...
		$dd_custom_width_px	 = apply_filters( 'daydream_header_image_width', $dd_custom_width_px );
		//define( 'HEADER_IMAGE_WIDTH', apply_filters( 'daydream_header_image_width', $dd_width_px ) );
		//define( 'HEADER_IMAGE_HEIGHT', apply_filters( 'daydream_header_image_height', 170 ) );
		//define( 'HEADER_TEXTCOLOR', '' );
		//define( 'NO_HEADER_TEXT', true );
	...
	$dd_width_layout = daydream_get_option( 'dd_width_layout', 'fixed' );
	...
	register_nav_menu( 'top-menu', __( 'Top Menu', 'daydream' ) );
...
function daydream_importer_admin_notice() {
	if ( isset( $_GET[ 'imported' ] ) && $_GET[ 'imported' ] == 'success' ) {
		...
		echo esc_html_e( 'Successfully imported demo data!', 'daydream' );
		echo "</p></div>";
	}
}
 ...
 * 
 */
function daydream_pll_get_post_types( $types ) {
	return array_merge( $types, array( 'daydream_portfolio' => 'daydream_portfolio', 'slide' => 'slide' ) );
}

add_filter( 'pll_get_post_types', 'daydream_pll_get_post_types' );
